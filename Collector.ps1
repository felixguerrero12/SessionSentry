...
try
{
        ...
        try
        {
            ...
            switch ($event.Id)
            {
                4624 {
                    ...
                    if ($logonType)
                    {
                        ...
                        Write-LogMessage "Login event for $username with LogonId $logonId - LogonType: $logonType" -Level Debug
                    }
                }
                4625 {
                    # Failed logon
            ...
            $eventRecord = [PSCustomObject]@{
                ...
                SecurityId = $eventXML.Event.EventData.Data |
                        Where-Object { $_.Name -eq 'TargetUserSid' } |
    ...
    if ($ShowExamples -and $Events.Count -gt 0)
    {
        ...
        $SortedEvents | Group-Object LogonId | Select-Object -First 3 | ForEach-Object {
            ...
            $_.Group | Format-Table Timestamp, EventType, Username, LogonType, LogonId, LinkedLogonId, SessionName, IsElevated -AutoSize
        }
    }
